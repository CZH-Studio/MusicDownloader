...
from music_platform import MusicPlatform, Music, MusicQueryResult, ResponseCode, Artist
...
class Bilibili(MusicPlatform):
    ...
    def query_keyword(self, keyword: str, page: int) -> MusicQueryResult:
        url = "https://api.bilibili.com/x/web-interface/search/type?" + (
            f"__refresh__=true"
            f"&_extra=&context="
            f"&page={page}&page_size={CONFIG.page_size}"
    ...
    def query_artist(self, artist_id: str) -> MusicQueryResult:
        url = f"https://api.bilibili.com/x/space/arc/search?mid={artist_id}&pn=1&ps=25&order=click&jsonp=jsonp"
        try:
            response = requests.get(
                url, headers=self.headers, cookies=self.cookies
